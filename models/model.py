...
class my_res50(nn.Module):
    """
    Modified to have num_classes=2 for binary classification (PTSD vs. non-PTSD).
    """
    def __init__(self, input_channels=3, num_classes=2, use_pretrained=True, state_dict_name='', root_dir='', mode="ir",
                 embedding_dim=512):
        super().__init__()
        ...
        # Changed out_features=2 for binary classification
...
class LeaderFollowerAttentionNetwork(nn.Module):
    """
    By default, we switch to 2 outputs for binary classification (change output_dim).
    """
    def __init__(self, backbone_state_dict, modality=['frame'], kernel_size=5, example_length=300, tcn_attention=0,
                 tcn_channel={'video': [512, 256, 256, 128], 'cnn_res50': [512, 256, 256, 128],
                              'mfcc': [32, 32, 32, 32], 'vggish': [32, 32, 32, 32]},
    ...
    def init(self):
        # Changed to 2 for binary classification
        self.output_dim = 2
...
class LeaderFollowerAttentionNetworkWithMultiHead(nn.Module):
    """
    First definition of this class with output_dim=2 for binary classification.
    """
    def __init__(self, backbone_state_dict, modality=['frame'], kernel_size=5, example_length=300, tcn_attention=0,
                 tcn_channel={'video': [512, 256, 256, 128], 'cnn_res50': [512, 256, 256, 128],
                              'mfcc':[32, 32, 32, 32], 'vggish': [32, 32, 32, 32]},
...
class LeaderFollowerAttentionNetworkWithMultiHead_v2(nn.Module):
    """
    Renamed the second definition to avoid overwriting the first one.
    Also uses output_dim=2 for binary classification.
    """
    def __init__(self, backbone_state_dict, modality=['frame'], kernel_size=5, example_length=300, tcn_attention=0,
                 tcn_channel={'video': [512, 256, 256, 128], 'cnn_res50': [512, 256, 256, 128],
                              'mfcc':[32, 32, 32, 32], 'vggish': [32, 32, 32, 32]},
...
class RCMA(nn.Module):
    """
    Modified to remove direct text usage (bert) if the goal is only audio+visual.
    Also changed output_dim to 2 for binary classification.
    """
    def __init__(self, backbone_settings, modality=['frame'], kernel_size=5, example_length=300, tcn_attention=0,
                 ...
                 embedding_dim={'video': 512,  'bert': 768, 'cnn_res50': 512,
                                'mfcc': 39, 'vggish': 128, 'logmel': 128, 'egemaps': 88},
                 encoder_dim={'video': 128, 'bert': 128, 'cnn_res50': 128,
                              'mfcc': 32, 'vggish': 32, 'logmel': 32, 'egemaps': 32},
                 modal_dim=32, num_heads=2,
                 root_dir='', device='cuda'):
        super().__init__()
        ...
        # For demonstration, we keep the original "final_dim" line but note it's not used exactly as in the multi-class case.
    ...
    def init(self):
        ...
        # Simple regressor with 2 outputs (instead of 1 with tanh).
    ...
    def forward(self, X):
        if 'video' in X:
            ...
            _, feature_dim = X['video'].shape
            X['video'] = X['video'].view(batch_size, length, feature_dim).unsqueeze(1)
        ...
        if 'logmel' in X:
            ...
            X['logmel'] = X['logmel'].view(batch_size, length, feature_dim).unsqueeze(1)

        # If you do NOT have text, comment out or remove references to X['bert']
        # For example, if your dataset doesn't have 'bert', remove it below:
        #
        # Before: video, audio, text = self.coattn(X['video'], X['logmel'], X['bert'])
        # We'll do a 2-input version:
        if 'video' not in X or 'logmel' not in X:
            raise ValueError("Expecting both 'video' and 'logmel' for this version of RCMA")

        # Using just (video, logmel)
        video, audio = self.coattn(X['video'].squeeze(1).transpose(1, 2),
                                   X['logmel'].squeeze(1).transpose(1, 2))
        # text is removed or commented out
        ...
        c = F.leaky_relu(c)
        c = self.regressor2(c)
        # Removed tanh for typical cross-entropy usage
        # c = torch.tanh(c)
...
class AttentionFusion(nn.Module):
    ...
    def __init__(self,
        ...
        for num_feats in num_feats_modality:
            self.attn.append(nn.Linear(num_feats, num_out_feats))

        self.weights = nn.Linear(num_out_feats * len(num_feats_modality),
                                 num_out_feats * len(num_feats_modality))
    ...
    def forward(self, x: list):
        ...
        attn_weights = F.softmax(self.weights(torch.cat(proj_m, -1)), dim=-1)
...
class CAN(nn.Module):
    """
    Example of a model now defaulting to 2 outputs for binary classification.
    """
    def __init__(self, modalities, tcn_settings, backbone_settings, output_dim, root_dir, device):
        ...
        # Changed final fc2 to produce 2 outputs
    ...
    def load_visual_backbone(self, backbone_settings):
        ...
        state_dict = torch.load(
            os.path.join(self.root_dir, backbone_settings['visual_state_dict'] + ".pth"),
            map_location='cpu'
        )
        resnet.load_state_dict(state_dict)
        for param in resnet.parameters():
    ...
    def load_audio_backbone(self, backbone_settings):
        vggish = AudioBackbone()
        state_dict = torch.load(
            os.path.join(self.root_dir, backbone_settings['audio_state_dict'] + ".pth"),
            map_location='cpu'
        )
        vggish.backbone.load_state_dict(state_dict)
        for param in vggish.parameters():
    ...
    def forward(self, X):
        ...
        # Example with coattn on just two modalities, 'logmel' and 'video'
        ...
        c = F.leaky_relu(c)
        c = self.fc2(c)
        # Removed tanh to allow standard cross-entropy usage
        return c
